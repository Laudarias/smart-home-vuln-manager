...
function Log($msg)      { Write-Host "[SHVM] $msg" -ForegroundColor Cyan    }
function LogOk($msg)    { Write-Host "[OK]   $msg" -ForegroundColor Green   }
function LogWarn($msg)  { Write-Host "[!]    $msg" -ForegroundColor Yellow  }
function LogError($msg) { Write-Host "[ERR]  $msg" -ForegroundColor Red     }
...
try {
    $testOutput = wsl.exe -e bash -c "echo ok" 2>&1
    if ($testOutput -match "ok") {
        $wslInstalled = $true
        LogOk "WSL2 disponible"
    }
} catch { }
...
if (-not $wslInstalled) {
    ...
    try {
        ...
        LogOk "WSL2 instalado."
        LogWarn "Se requiere reiniciar el equipo para continuar."
        LogWarn "Después de reiniciar, vuelve a ejecutar el instalador."
        exit 0
    } catch {
        LogError "No se pudo instalar WSL2 automáticamente."
        LogError "Instálalo manualmente ejecutando en PowerShell (administrador): wsl --install"
        exit 1
    }
}
...
# ─── Helpers: ejecutar comandos en WSL2 ──────────────────────────────────────
...
if ($userExists -match "^\d+$") {
    ...
    Wsl-Run "useradd -m -s /bin/bash $WslUser" | Out-Null
    Wsl-Run "echo '${WslUser}:${WslUser}' | chpasswd" | Out-Null
...
$deps = "python3 python3-pip python3-venv git arp-scan nmap unzip curl"
...
if ($nucleiInstalled -match "/nuclei") {
    LogOk "Nuclei ya está instalado"
} else {
    Wsl-Run "curl -sL https://github.com/projectdiscovery/nuclei/releases/latest/download/nuclei_linux_amd64.zip -o /tmp/nuclei.zip && unzip -q /tmp/nuclei.zip nuclei -d /usr/local/bin && chmod +x /usr/local/bin/nuclei && rm /tmp/nuclei.zip" | Out-Null
...
if ($repoExists -match "yes") {
    ...
    Wsl-Run "chown -R ${WslUser}:${WslUser} $InstallDir" | Out-Null
...
$sudoersLine = "$WslUser ALL=(ALL) NOPASSWD: /usr/sbin/arp-scan, /usr/bin/nmap"
Wsl-Run "echo '$sudoersLine' > /etc/sudoers.d/shvm && chmod 440 /etc/sudoers.d/shvm" | Out-Null
...
if (-not ($nodeInstalled -match "/node")) {
    Log "Node.js no encontrado, instalando..."
    Wsl-Run "curl -fsSL https://deb.nodesource.com/setup_20.x | bash - && apt-get install -y -qq nodejs" | Out-Null
    LogOk "Node.js instalado"
}
...
Write-Host "========================================" -ForegroundColor Green
Write-Host ""
Write-Host "La app Smart Home Vulnerability Manager esta lista." -ForegroundColor White
Write-Host "Abrela desde el acceso directo del escritorio."      -ForegroundColor White
Write-Host ""